...
class MainScreenViewModel @Inject constructor(
    ...
    fun onEvent(event: MainScreenEvent) {
        when(event) {
            MainScreenEvent.FromCurrencySelect -> {
                state = state.copy(selection = SelectionStates.FROM)
            }
            MainScreenEvent.ToCurrencySelect -> {
                state = state.copy(selection = SelectionStates.TO)
            }
            ...
            is MainScreenEvent.NumberButtonItemClicked -> {
                updateCurrencyValue(value = event.value)
            }
            is MainScreenEvent.BottomSheetItemClicked -> {
                if (state.selection == SelectionStates.FROM) {
                    state = state.copy(fromCurrencyCode = event.value)
                } else if (state.selection == SelectionStates.TO) {
    ...
    private fun updateCurrencyValue(value: String) {
        ...
        val currentCurrencyValue = when(state.selection) {
            SelectionStates.FROM -> state.fromCurrencyValue
            SelectionStates.TO -> state.toCurrencyValue
        }
